...
import cc.altius.williamsChicken.model.rowMapper.CustomUserDetailsRowMapper;
...
import cc.altius.williamsChicken.model.rowMapper.RoleRowMapper;
import cc.altius.williamsChicken.model.rowMapper.UserRowMapper;
...
public class UserDaoImpl implements UserDao {
    ...
    public int updateFailedAttemptsByUserId(String userName) {
        try {
            String sqlQuery = "UPDATE `user` SET FAILED_ATTEMPTS=FAILED_ATTEMPTS+1 WHERE EMAIL_ID=?";
    ...
    public User getUserByUsername(String username) {
        String sqlString = "SELECT user.*,user_role.* ,role.* FROM user "
                + " LEFT JOIN user_role ON user.USER_ID=user_role.USER_ID "
                + " LEFT JOIN role ON user_role.ROLE_ID=role.ROLE_ID  "
                //                + " LEFT JOIN user_type ut ON ut.`USER_TYPE_ID`=user.`USER_TYPE_ID`"
                //                + " LEFT JOIN user_status us ON us.`STATUS_ID`=user.`USER_STATUS_ID`"
                + " WHERE user.`EMAIL_ID`=?";
